<!DOCTYPE html>

<html lang="PT-BR">
<head>
  <link rel="stylesheet" href="cssperfil/navbarperfil.css">
  <link rel="stylesheet" href="cssperfil/perfilCadastro.css">
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Perfil - Aluno</title>
  <link rel="stylesheet" href="css/bootstrap.min.css" type="text/css">
  <link href="css/mdb.min.css" rel="stylesheet">
  <link rel="stylesheet" href="path/to/font-awesome/css/font-awesome.min.css">
</head>

<body style="background-color: rgb(248, 248, 248);">
<nav class="navbar navbar-expand-lg navbar navbar-light" style="background-image: linear-gradient(to left, rgba(169, 210, 233, 0.8), rgba(139, 192, 223, 0.8), rgba(183, 210, 226, 0.8))">
  <!-- Container wrapper -->
  <div class="container-fluid">
    <button
      class="navbar-toggler"
      type="button"
      data-mdb-toggle="collapse"
      data-mdb-target="#navbarRightAlignExample"
      aria-controls="navbarRightAlignExample"
      aria-expanded="false"
      aria-label="Toggle navigation"
    >
      <i class="fas fa-bars"></i>
    </button>

    <div class="collapse navbar-collapse" id="navbarRightAlignExample">
		<div class="collapse navbar-collapse" id="navbarLeftAlignExample">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="../../../../../Mundos/index.html"><img src="img/LogoCodeLearning.png" height="60px" width="100px"/></a>
        </li>
		</ul>
		</div>
      <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active text-white" aria-current="page" href="#"><img src="img/group-icon.png"/>Sobre nós</a>
        </li>
        <li class="nav-item">
          <a class="nav-link  text-white" href="#"><img src="img/ajuda-icon.png"/>Ajuda</a>
        </li>
      </ul>
    </div>
  </div>
</nav>

  <div id="perfilAlunoEs">

  <!-- PARTE 1 -->
    <div class="headerPerfil">
      <div class="conteudoPerfil">
        <p class="tituloHeaderPerfil"> Perfil</p>   
        <div class="grid-headerPerfil">
          <?php
              
              include("conexao.php");
              include("verificacao/verificacao-login-aluno.php");

              $id = $_SESSION['id'];

              try {
                
                $stmt = $pdo->prepare("select * from tbCadastroAluno  where idCadastroAluno=$id ");
                $stmt->execute();
            
                while ($row = $stmt->fetch(PDO::FETCH_BOTH)) {
                  echo "<div scope='row'><img src='img/anonima.jpg' class='imgPerfil'/></div>";
                  echo "<div class='dadosHeaderPerfil'>";
                    $row['idCadastroAluno'];
                    echo "<p class='nomeheaderPerfil'>".$row['nomeAluno']."</p>";
                    echo "<p class='emailheaderPerfil'>".$row['emailAluno']."</p>";
                    $row['senhaAluno'];
                    
                    echo "<a href='?idAl=$row[idCadastroAluno]&nomeAl=$row[nomeAluno]&emailAl=$row[emailAluno]&senhaAl=$row[senhaAluno]'>";
                        echo "Alterar Dados";
                    echo "</a>";
                  echo "</div>";
                }
            
            } catch (PDOException $e) {
                echo "Erro" . $e -> getMessage();
            }

          ?>
        </div>
      </div>
    </div>

    <!-- PARTE 2 -->
    <div class="conteudoPerfil">
      <p class="tituloHeaderPerfil">Alteração de dados</hp>    
      <div class="grid-corpoPerfil">
        <!-- primeira coluna -->
        <div>
          <form>
            <label for="fotoAl"> Foto
              <input type="file" id="fotoAl" name="fotoAl">
            </label>
          </form>

          <form action="alteracao/alterar-dados-aluno.php" method="post">
            <label for="inpuId">ID
              <input type="text" class="inputAlterar" id="inpuId" placeholder="ID Aluno" name="txIdAluno" value="<?php echo @$_GET['idAl']; ?>">
            </label>

            <label for="inputNome">Nome
              <input type="text" class="inputAlterar" id="inputNome" placeholder="Nome Aluno" name="txNomeAluno" value="<?php echo @$_GET['nomeAl']; ?>">
            </label>

          <input type="submit" class="btnAlterar" placeholder="Enviar">
        </div>
        <!-- fim primeira coluna -->

        <!-- segunda coluna -->
        <div>
          <label for="inpuEmail">Email
            <input type="email" class="inputAlterar" id="inpuEmail" placeholder="Email Aluno" name ="txEmail" value="<?php echo @$_GET['emailAl']; ?>">
          </label>

          <label for="inputSenha">Senha
            <input type="password" class="inputAlterar" id="inputSenha" placeholder="Nova Senha" name="txSenhaAl" value="<?php echo @$_GET['senhaAl']; ?>">
          </label>

          <label for="inputConfSenha">Confirme sua senha
            <input type="password" class="inputAlterar" id="inputConfSenha" placeholder="Confirmar senha" name ="txCSenhaAl">
          </label>

        </div>
        <!-- fim segunda coluna -->

        </form>
      </div>
    </div>
  </div>

<!-- <br><br> -->
<!-- Footer -->
<footer class="text-center text-white" style="background-image: linear-gradient(to left, rgba(169, 210, 233, 0.8), rgba(139, 192, 223, 0.8), rgba(183, 210, 226, 0.8))">
    <div class="container p-4">
      <section class="">
        <div class="row">
          <div class="col-lg-6 col-md-6 mb-6 mb-md-0">
            <h5 class="text-uppercase"> Fale Conosco</h5>
            <div id="tracoQmSms"></div>
  
            <ul class="list-unstyled mb-0">
              <li> 
           <img src="img/email-icon.png"/> contatobachstech@gmail.com</i>
              </li>
              <li>
                <img src="img/tel-icon.png"/> +55 4002 - 8922</i>
              </li>
              <li>
          <img src="img/group-icon.png"/> 11 96314 - 2524</i>
              </li>
            </ul>
          </div>

          <div class="col-lg-6 col-md-6 mb-6 mb-md-0">
              <h5 class="text-uppercase"> Quem Somos</h5>
              <div id="tracoQmSms"></div>

            <ul class="list-unstyled mb-0">
              <li>
                <img src="img/youtube-icon.png"/> Bachstech Company</i>
              </li>
              <li>
            <img src="img/twitter-icon.png"/> @Bachstech</i>
              </li>
              <li>
                <img src="img/insta-icon.png"/> Bachstech
              </li>
              <li>
                <img src="img/github-icon.png"/> Bachstech Company</i>
              </li>
            </ul>
          </div>
        </div>
      </section>
    </div>
    <div
      class="text-center p-3"
      style="background-color: rgba(0, 0, 0, 0.2)">
      © 2021 Copyright:
      <a class="text-white" href="https://mdbootstrap.com/">Bachstech</a>
    </div>
  </footer>
<!-- Footer -->
  </body>
</html>